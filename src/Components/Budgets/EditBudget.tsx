...
interface Props {
  ...
  budget: Budgets[];
  setBudget: React.Dispatch<React.SetStateAction<Budgets[]>>;
  editingIndex: number;
...
const EditBudget: React.FC<Props> = ({
  category,
  budget,
  setBudget,
  editingIndex,
  ...
  const handleSave = (category: string | undefined) => {
    if (!category) return;

    const updatedBudgets = budget.map((budget, i) =>
      editingIndex === i
        ? {
            ...budget,
            theme: selectedColor,
    ...
    <div>
      {isEditing && (
        <div className="modal-overlay">
          <div className="modal w-[30%]  p-8 flex flex-col gap-5 rounded-[12px] ">
            <div className="flex justify-between">
              <p className="text-1">Edit Budget</p>
            ...
            <div>
              ...
              <select
                ...
                className="w-full border border-black rounded-[12px] px-5 py-2 "
                name="themePicker"
              >
                {data.category.map((c) => (
                  <option key={c.id} value={c.category} className="border-b">
                    {c.category}
                  </option>
                ))}
              </select>
            ...
            <div>
              ...
              <select
                ...
                className="w-full border border-black px-5 py-2 "
                name="themePicker"
              >
                {data.color.map((color, i) => (
                  <option key={i} value={color.hex} className="border-b">
                    {color.name}
                  </option>
                ))}
              </select>
